...
resource "aws_s3_bucket" "aman-bucket" {
  bucket = "aman-static-data-bucket"
  acl    = "public-read"
}

//Putting Objects in S3 Bucket
resource "aws_s3_bucket_object" "web-object1" {
  bucket = "${aws_s3_bucket.aman-bucket.bucket}"
  key    = "aman.jpg"
...
resource "aws_cloudfront_distribution" "s3-web-distribution" {
  origin {
    domain_name = "${aws_s3_bucket.aman-bucket.bucket_regional_domain_name}"
    origin_id   = "${aws_s3_bucket.aman-bucket.id}"
  }
  ...
  default_cache_behavior {
    ...
    target_origin_id = "${aws_s3_bucket.aman-bucket.id}"
